...
By default, this tool creates two files related to attributions:
...
You can opt-out of this behavior by passing the `--encode-versions` flag. When this is set, package version
information is embedded inside the `THIRD_PARTY_LICENSES` file, and the `THIRD_PARTY_LICENSES.versions.json` will
not be generated. Note that in such a setup, any version upgrade will cause a validation error that would require
a regeneration of this file. In addition, it can 
